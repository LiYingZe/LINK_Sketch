...
def calculate_qerror_metrics(qerrors):
    ...
    metrics = {
        'median': np.percentile(qerrors, 50),  # 50th percentile (median)
...
def evaluate_predictionSingle(sl, args):
    ...
    try:
        print(f"True frequency: {int(actualFrep[args.word])}")
        print(f"Est frequency: {int(predictFreq[args.word])}")
        print(f"Qerror: {max((actualFrep[args.word]+1)/(predictFreq[args.word]+1), (predictFreq[args.word]+1)/(actualFrep[args.word]+1))}")
    except:
        print(f"The word {args.word} is not in this corpus.")
    print("---"*30)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--word', type=str, default="the", help='The query word.')
